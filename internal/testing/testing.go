...
type configTemplate struct {
	...
	followRequestUserPrompt func(string) bool
	followReplyDisplay      func(string)
	flagThreshold           int
	rpcRetry                int
}
...
func WithRequestValidation(rv bool) Option {
	return func(ct *configTemplate) {
		ct.requestValidation = rv
	}
}

func WithChord(c bool) Option {
	return func(ct *configTemplate) {
		ct.withChord = c
	}
}
...
func WithChallengeTimeout(d time.Duration) Option {
	return func(ct *configTemplate) {
		ct.challengeTimeout = d
	}
}

func WithFollowRequestUserPrompt(f func(string) bool) Option {
	return func(ct *configTemplate) {
		ct.followRequestUserPrompt = f
	}
}

func WithPostEncryption(pe bool) Option {
	return func(ct *configTemplate) {
		ct.postEncryption = pe
	}
}
...
func NewTestNode(t require.TestingT, f peer.Factory, trans transport.Transport,
	...
	config.RequestValidation = template.requestValidation
	config.WithChord = template.withChord
	config.ChallengeTimeout = template.challengeTimeout
	config.FollowRequestUserPrompt = template.followRequestUserPrompt
	config.FollowReplyDisplay = template.followReplyDisplay
	config.PostEncryption = template.postEncryption
	config.FlagThreshold = template.flagThreshold
	config.RPCRetry = template.rpcRetry
...
// GetFollowRequest returns the Private message associated to the transport.Message.
func GetFollowRequest(t *testing.T, msg *transport.Message) types.FollowRequestMessage {
	require.Equal(t, "followrequest", msg.Type)
...
// GetFollowReply returns the Private message associated to the transport.Message.
func GetFollowReply(t *testing.T, msg *transport.Message) types.FollowReplyMessage {
	require.Equal(t, "followreply", msg.Type)

	var followReplyMessage types.FollowReplyMessage

	err := json.Unmarshal(msg.Payload, &followReplyMessage)
	require.NoError(t, err)
...
// DisplayReachability displays what posts each node can reach
func DisplayReachability(t *testing.T, out io.Writer, reachability [][]error, crashed []int) {
	fmt.Fprint(out, "\t")
	for i := range reachability[0] {
		fmt.Fprint(out, "post", i, "\t")
	}
	fmt.Fprint(out, "\n")

	for userNumber, userReachability := range reachability {
		fmt.Fprint(out, "user", userNumber, "\t")
		if contains(crashed, userNumber) {
			fmt.Fprint(out, "---crashed---\n")
			continue
		}
		for _, err := range userReachability {
			if err != nil {
				fmt.Fprint(out, " - \t")
			} else {
				fmt.Fprint(out, " O \t")
